...
export function calculateTaxes(
  baseAmount: number,
  vatTax: TaxSetting | null,
  serviceTax: TaxSetting | null
 ...
 */
export function getTaxDescription(tax: TaxSetting | null): string {
  if (!tax?.enabled) return 'Not applicable';
  
  if (tax.type === 'percentage') {
    return `${tax.value}%`;
  } else {
    return `${String.fromCharCode(0xe001)}${tax.value.toFixed(2)} fixed`;
  }
} 